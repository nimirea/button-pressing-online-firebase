  ...
  data: () => {
    return {
      ...
      modules: [ // list of modules to show
        ...
        {
          text: [
            'Just like before, you\'ll be pressing the keys within each pane in the same color order: <span class="red">red</span> first, then <span class="blue">blue</span>, then <span class="gray">gray</span>.',
        ...
        {
          text: [
            'Nice! We\'ll remind you of this order again from time to time during the experiment.'
          ],
          completionCondition: {
            sequence: ['left thumb', 'right thumb'],
            simultaneous: true,
            text: "Press both thumbs to continue to the experiment. The first trial will start automatically."
          },
          days_shown: [2, 3, 4]
        }
      ],
        ...
        for (let m = 0, len = this.modulesShownToday.length; m < len; m++) {
          ...
          if (this.modulesShownToday[m].completionCondition.sequence.length === 0) {
            ...
            if (this.modulesShownToday[m].tableau != undefined) {
              ...
              let panes = this.modulesShownToday[m].tableau.stimRef.split(" ")
              panes.map((pane) => {
